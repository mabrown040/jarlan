...
**Schema prereqs:** v2 → v3 bump (adds `Scenario.meta` for source provenance + assumption log)
...
Migration in [src/lib/domain/migrations.ts](../src/lib/domain/migrations.ts): `v2_to_v3` is a no-op for existing scenarios (`meta` defaults to `undefined`). Round-trip test added to [migrations.test.ts](../src/lib/domain/__tests__/migrations.test.ts).

Cloud scenarios stored as v2 will arrive without `meta` until re-synced — handled by the migration as the gate per the existing pattern at [src/lib/supabase/sync.ts](../src/lib/supabase/sync.ts).
...
- [ ] Bump Scenario schema v2 → v3 (`meta` field) with migration + round-trip test
